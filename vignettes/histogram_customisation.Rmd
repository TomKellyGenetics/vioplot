...
vignette: >
  %\VignetteIndexEntry{histoplot: Customising Violin Plots}
...
Therefore histogram plots are a powerful tool to assist researchers to visualise data, particularly in the quality checking and exploratory parts of an analysis. Violin plots have many benefits:
...
library("histoplot")
histoplot(iris$Sepal.Length[iris$Species=="setosa"], iris$Sepal.Length[iris$Species=="versicolor"], iris$Sepal.Length[iris$Species=="virginica"], names=c("setosa", "versicolor", "virginica"))
```
...
Plot colours can be further customised as with the original histoplot package using the `col` argument:
...
However the `histoplot` (0.2) function is unable to colour each histogram separately, thus this is enabled with a vectorised `col` in `histoplot` (0.3):
...
This should be sufficient to customise the histogram plot but further examples are given in [the areaEqual histoplot vignette](histogram_area.html) including how histogram plots are useful for comparing variation when data does not follow the same distribution. This document also compares the histogram plot with other established methods to plot data variation.
